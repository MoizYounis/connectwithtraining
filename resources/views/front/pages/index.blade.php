    ...
    <main id="maincontent" class="page-main">
                    ...
                    <div class="popup-padd-top-bot">
                        ...
                        <div class=""><br>
                            ...
                            <div class="form-group home-popup-design">
                                <div>
                                    <input type="email" class="form-field subEmail" name="email" placeholder="Email"
                                        value="" required="">
                                </div>
                                <div>
                                    <input type="submit" class="btn" value="Sign Up"
                                        onclick="doSubscribe1('{{ csrf_token() }}')">
                                </div>
                            </div>
                        </div>
                        <p style="color: #fff; margin-top: 7px;"><a href="{{ url('terms-conditions') }}"
                                style="text-decoration: none; color: white;">Terms and Conditions</a> <span
                                style="margin-left: 9px;">|</span> <a href="{{ url('privacy-policy') }}"
                                style="text-decoration: none; color: white; margin-left: 10px;">Privacy Policy</a></p>
                        <!--</form>-->
                    </div>
                </div>
            </div>
        ...
        <div id="myModal2" class="modal" style="z-index: 10243;">
            ...
            <div class="modal-content modal-home-popup2 referel-modal">
                <div class="ref_mod">
                    <a href="https://www.connectwithtraining.com/refer-a-friend"><img class="home-popup-img2"
                            src="{{ asset('public/assets/front/images/home-popup1.png') }}">
                    </a>
        ...
        <div class="columns">
            <div class="column main">
                <div class="fixed-live-chart">
                    <ul>
                        ...
                        <li class="fxd-contact-cht"><a href="{{ url('contact') }}">Contact With Us</a></li>
                    </ul>
                </div>
                ...
                @if ($data->banner_img != '')
                    <div class="banner-outer">
                        <ul>
                            <li>
                                <img src="{{ asset('public/assets/front/img/banner/' . $data->banner_img) }}"
                                    alt="" />
                                <div class="ban-con">
                                    <h2 class="wel">{{ isset($data->text_on_banner) ? $data->text_on_banner : '' }}
                                    </h2>
                                </div>
                            </li>
                        </ul>
                    ...
                    <div class="disnt-outer">
                        <div class="disnt-inr">
                            <div class="dis-con">
                                <span
                                    class="disnt-price">{{ isset($data->banner_footer_text) ? $data->banner_footer_text : '' }}
                    ...
                    <div class="toolbar toolbar-products top homepage">
                        <div class="left">
                            <div class="brows-drop">
                                <!-- 30-06-2022 -->
                                <select id="category" style="width:175px;" onfocus='this.size=25;' onblur='this.size=0;'
                                    onchange='this.size=1; this.blur();' class="select-category border">
                                    <!--style="border-style: solid; border-width: 5px; border-color: black;" -->
                                        <option value="">Select Category</option>
                                        @foreach ($category_data as $cat)
                                            <option value="{{ $cat->id }}">{{ ucwords($cat->category_name) }}
                                            </option>
                                        @endforeach
                                </select>
                            </div>
                        ...
                        <div class="right">
                            ...
                            <div class="toolbar-sorter sorter">
                                ...
                                <ul class="price-range">
                                    ...
                                    ?>
                                    <li data-amount="0,{{ $priceDiv }}">
                                        < {{ Helper::changeCurrency($priceDiv) }} </li>
                                    <li data-amount="{{ $priceDiv + 1 }},{{ $priceDiv * 2 }}">
                                        {{ Helper::changeCurrency($priceDiv + 1) }} -
                                        {{ Helper::changeCurrency($priceDiv * 2) }}</li>
                                    <li data-amount="{{ $priceDiv * 2 + 1 }},{{ $priceDiv * 3 }}">
                                        {{ Helper::changeCurrency($priceDiv * 2 + 1) }} -
                                        {{ Helper::changeCurrency($priceDiv * 3) }}</li>
                                    <li data-amount="{{ $priceDiv * 3 + 1 }},{{ $price->max_price }}">
                                        {{ Helper::changeCurrency($priceDiv * 3 + 1) }} +</li>
                            ...
                            <div class="brows-drop pops-drop">
                                ...
                                <!-- 30-06-2022 -->
                                <select class="selected-1 select-pops" style=" z-index: 10242 !important;width: 230px;"
                                    id="change-cat" name="2" onfocus='this.size=5;' onblur='this.size=0;'
                                    onChange="this.size=1; this.blur(); myFunction(this); filterCourse({courseStatus: this.options[this.selectedIndex].value}, null, '.traning-outer ul');">
                                    <option style="text-align: center;" value=""> &nbsp;&nbsp;&nbsp;Popular
                                        Today
                                    </option>
            ...
            @if ($data->homepage_middle_img != '')
                <div class="expo-outer"
                    style="background: url({{ asset('public/assets/front/img/banner/' . $data->homepage_middle_img) }}) no-repeat 0 0;">
                    <div class="container">
                        <h2>{{ isset($data->minddle_img_title) ? $data->minddle_img_title : '' }}</h2>
                        <p class="get-eposer">
                            {{ isset($data->minddle_img_subtitle) ? $data->minddle_img_subtitle : '' }}
                        </p>
                    </div>
                </div>
            @endif
                ...
                <div class="container">
                    ...
                    <div class="teacher-outer">
                        <ul>
                            ...
                            $i = 0;
                            $userReview = [
                                'review1' => ['img' => $data->review_images[0], 'title' => $data->review_title1, 'comment' => $data->review_comment1, 'rate' => $data->review_rating1],
                                'review2' => ['img' => $data->review_images[1], 'title' => $data->review_title2, 'comment' => $data->review_comment2, 'rate' => $data->review_rating2],
                                'review3' => ['img' => $data->review_images[2], 'title' => $data->review_title3, 'comment' => $data->review_comment3, 'rate' => $data->review_rating3],
                                'review4' => ['img' => $data->review_images[3], 'title' => $data->review_title4, 'comment' => $data->review_comment4, 'rate' => $data->review_rating4],
                            ];
                            ?>
                            @foreach ($userReview as $review)
                                <li>
                                    <div class="pro-img">
                                        <img src="{{ asset('public/assets/front/img/publicReviews/') . '/' . $review['img'] }}"
                                            alt="{{ $review['title'] }}">
                                    ...
                                    <div class="rating-outer">
                                        <div class="rating-star" style="text-align:center;">
                                            <span
                                                class="fa fa-star @if ($review['rate'] >= 1) {{ 'checked' }} @endif"></span>
                                            <span
                                                class="fa fa-star @if ($review['rate'] >= 2) {{ 'checked' }} @endif"></span>
                                            <span
                                                class="fa fa-star @if ($review['rate'] >= 3) {{ 'checked' }} @endif"></span>
                                            <span
                                                class="fa fa-star @if ($review['rate'] >= 4) {{ 'checked' }} @endif"></span>
                                            <span
                                                class="fa fa-star @if ($review['rate'] >= 5) {{ 'checked' }} @endif"></span>
                                        </div>
                                    </div>
                                    <div class="des">
                                        <p>{{ $review['comment'] }}</p>
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            getRelatedCourse({
                courseStatus: "Featured"
            }, '.featuredCourse', '6', '2', '{{ csrf_token() }}');
        });
