...
@section('title')Product List | Products @endsection
@section('content')
        ...
        <div class="row pt-2 pb-2">
            ...
            <div class="col-sm-3">
                <div class="btn-group float-sm-right">
                    <a type="button" href="{{route('product.create')}}" class="btn btn-behance text-white btn-sm waves-effect waves-light add_data"><i class="fa fa-plus mr-1"></i> Add </a>
                </div>
            </div>
        </div>        
                                ...
                                <tbody>
                                    @foreach($product_list as $row)
                                        <tr>
                                            <td>{{$row->id}}</td>
                                            <td>
                                              <b>{{ucwords($row->product_name)}}</b><br>
                                              {{(ucwords($row->category_name.' / '.$row->sub_name.' / '.$row->child_name))}}
                                            </td>
                                            <td><img src="{{asset('public/assets/front/img/product')}}/{{$row->product_base_img}}" class="img img-circle" height="50" width="50"></td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="btn-group m-1" role="group">
                                                    ...
                                                    <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 37px, 0px);">
                                                        <a href="{{route('product.edit',[$row->id])}}" class="dropdown-item edit_data"><i class="fa fa-edit mr-1"></i> Edit Data</a>
