...
@section('title')Update Blog | Blogs @endsection
@section('content')
      ...
      <!-- Breadcrumb-->
        ...
        <div class="row">
            <div class="col-lg-12">
                <div id="messages"></div>
                <div class="card">
                    <div class="card-header"><i class="zmdi zmdi-reader"></i> Update Blog</div>
                    <div class="card-body">
                        <form method="post" action="{{route('blog.update', [$blog_edit->id])}}">
                            @csrf
                            {{ method_field('PATCH') }}
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <select name="blog_cat_id" class="form-control input-check select2" id="blog_cat_id" required="">
                                        ...
                                        @foreach($blog_cat_list as $cat)
                                            @if($blog_edit->blog_cat_id == $cat->id)
                            ...
                            <div class="row">
                              <div class="form-group col-md-12">
                                ...
                                <input type="text" class="form-control input-check" name="blog_title" id="blog_title" placeholder="Enter Blog Title" value="{{$blog_edit->blog_title}}" required="">
                              </div>

                              <div class="form-group col-md-12">
                                <label>Blog Content*</label>
                                <textarea id="summernoteEditor" name="blog_content" required="">{{$blog_edit->blog_content}}</textarea>
                              </div>
                            </div>
                            ...
                            <div class="row">
                              <div class="form-group col-md-12">
                                ...
                                <div class="icheck-material-primary">
                                    ...
                                    @if($blog_edit->blog_status == "Publish") checked="" @else @endif
                                    />
                            ...
                            <?php
                               $blog_meta_key = explode(',', $blog_edit->blog_meta_key);
                               $blog_meta_value = explode(',', $blog_edit->blog_meta_value);
                               $i=0;
                            ?>

                            @while($i < count($blog_meta_key))
                                <div class="row" id="inputFormRow">
                                    <div class="form-group col-md-6">
                                        <label>Blog Meta Key</label>
                                        <textarea class="form-control input-check" name="blog_meta_key[]" id="blog_meta_key" placeholder="Enter Blog Meta Key">{{$blog_meta_key[$i]}}</textarea>
                                    </div>

                                    <div class="form-group col-md-6">
                                        <label>Blog Meta Value</label>
                                        <textarea class="form-control input-check" name="blog_meta_value[]" id="blog_meta_value" placeholder="Enter Blog Meta Value">{{$blog_meta_value[$i]}}</textarea>
                                        @if($i > 0)
